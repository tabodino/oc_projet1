...
<div id="" class="featured-post-info">
	...
	<?php 
		if( has_post_thumbnail() ){
			$image = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'featured-thumbnail', false ); 
			set_post_thumbnail_size( 350, 245, 1 );
		?>
		<img src="<?php echo esc_url($image[0]); ?>" alt="<?php the_title(); ?>">
		<?php }else {?>	
	...
	<div class="featured-content">
		<p><?php echo accesspresslite_excerpt( get_the_content() , 400 ) ?></p>
		<a href="<?php the_permalink(); ?>" class="cat-event-more bttn">Découvrir</a>
	</div>
<br>
...
<?php elseif(!empty($cat_portfolio) && is_category(6) && is_category($cat_portfolio)): ?>

<tr>

	<td><?php echo get_post_meta($post->ID, 'Date de l\'activité', true) ?></td>
	...
	<td><a href="#" class="cat-event-more bttn eModal-">S'inscrire</a></td>
